...
cmd:option('-train_par', '', [[Path to the training paragraph data]])
cmd:option('-valid_src', '', [[Path to the validation source data]])
cmd:option('-valid_tgt', '', [[Path to the validation target data]])
cmd:option('-valid_par', '', [[Path to the validation paragraph data]])
...
cmd:option('-par_vocab_size', 50000, [[Size of the paragraph vocabulary]])
cmd:option('-src_vocab', '', [[Path to an existing source vocabulary]])
cmd:option('-tgt_vocab', '', [[Path to an existing target vocabulary]])
cmd:option('-par_vocab', '', [[Path to an existing paragraph vocabulary]])
...
cmd:option('-par_seq_length', 50, [[Maximum paragraph sequence length]])
...
local function makeData(srcFile, tgtFile, parFile, srcDicts, tgtDicts, parDicts)
  local src = tds.Vec()
  ...
  local parReader = onmt.utils.FileReader.new(parFile)
  ...
  while true do
    ...
    if srcTokens == nil or tgtTokens == nil or parTokens == nil then
      if srcTokens ~= nil or tgtTokens ~= nil or parTokens ~= nil then
        print('WARNING: source, target, paragraph do not have the same number of sentences')
      end
      break
    end

    if #parTokens > opt.par_seq_length then
      local tmp = parTokens
      parTokens = {}
      for i = 1, opt.par_seq_length do
        parTokens[i] = tmp[i]
      end
    end

    if #srcTokens > 0 and #srcTokens <= opt.src_seq_length
    and #tgtTokens > 0 and #tgtTokens <= opt.tgt_seq_length
    and #parTokens > 0 and #parTokens <= opt.par_seq_length then
      ...
      par:insert(parDicts.words:convertToIdx(parWords, onmt.Constants.UNK_WORD))
      ...
      if #parDicts.features > 0 then
        parFeatures:insert(onmt.utils.Features.generateTarget(parDicts.features, parFeats, true))
      end
  ...
  local function reorderData(perm)
    ...
    par = onmt.utils.Table.reorder(par, perm, true)
    ...
    if #parDicts.features > 0 then
      parFeatures = onmt.utils.Table.reorder(parFeatures, perm, true)
    end
  end
  ...
  print('Prepared ' .. #src .. ' sentences (' .. ignored
          ...
          .. ' or paragraph length > ' .. opt.par_seq_length .. ')')
  ...
  local parData = {
    words = par,
    features = parFeatures
  }

  return srcData, tgtData, parData
end
