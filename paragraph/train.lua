...
cmd:option('-para_rnn_size', 250, [[Size of Paragraph LSTM hidden states]])
cmd:option('-sent_rnn_size', 250, [[Size of Sentence LSTM hidden states]])
...
cmd:option('-para_sent_encoder', false, [[Use another paragraph-level encoder]])
...
cmd:option('-pre_word_vecs_sent_enc', '', [[If a valid path is specified, then this will load
                                          pretrained word embeddings on the sentence encoder side.
                                          See README for specific formatting instructions.]])
cmd:option('-pre_word_vecs_para_enc', '', [[If a valid path is specified, then this will load
                                          pretrained word embeddings on the paragraph encoder side.
                                          See README for specific formatting instructions.]])
cmd:option('-pre_word_vecs_dec', '', [[If a valid path is specified, then this will load
                                     pretrained word embeddings on the decoder side.
                                     See README for specific formatting instructions.]])
cmd:option('-fix_word_vecs_sent_enc', false, [[Fix word embeddings on the sentence encoder side]])
cmd:option('-fix_word_vecs_para_enc', false, [[Fix word embeddings on the paragraph encoder side]])
...
local function main()
  ...
  if opt.train_from:len() > 0 then
    ...
    opt.para_sent_encoder = checkpoint.options.para_sent_encoder
  ...
  local trainData = onmt.data.Dataset.new(dataset.train.src, dataset.train.tgt, dataset.train.par)
  local validData = onmt.data.Dataset.new(dataset.valid.src, dataset.valid.tgt, dataset.valid.par)

  trainData:setBatchSize(opt.max_batch_size)
  validData:setBatchSize(opt.max_batch_size)

  if not opt.json_log then
    print(string.format(' * vocabulary size: source = %d; target = %d; paragraph = %d',
                        dataset.dicts.src.words:size(), dataset.dicts.tgt.words:size(), dataset.dicts.par.words:size()))
    print(string.format(' * additional features: source = %d; target = %d; paragraph = %d',
                        #dataset.dicts.src.features, #dataset.dicts.tgt.features, #dataset.dicts.par.features))
    print(string.format(' * maximum sequence length: source = %d; target = %d; paragraph = %d',
                        trainData.maxSourceLength, trainData.maxTargetLength, trainData.maxPrgrphLength))
  ...
  onmt.utils.Parallel.launch(function(idx)
    ...
    if checkpoint.models then
      if checkpoint.options.para_sent_encoder then
        _G.model.encoder = onmt.Models.loadParaSentEncoder(checkpoint.models.encoder, idx > 1)
        _G.model.decoder = onmt.Models.loadParaSentDecoder(checkpoint.models.decoder, idx > 1)
      else
      ...
      local verbose = idx == 1 and not opt.json_log
      if opt.para_sent_encoder then
        _G.model.encoder = onmt.Models.buildParaSentEncoder(opt, dataset.dicts.src, dataset.dicts.par)
        _G.model.decoder = onmt.Models.buildParaSentDecoder(opt, dataset.dicts.tgt, verbose)
      else
