...
local function buildParaSentEncoder(opt, sentDicts, paraDicts)
  local inputSentNetwork, inputSentSize = buildInputNetwork(opt, sentDicts, opt.pre_word_vecs_sent_enc, opt.fix_word_vecs_sent_enc)
  local inputParaNetwork, inputParaSize = buildInputNetwork(opt, paraDicts, opt.pre_word_vecs_para_enc, opt.fix_word_vecs_para_enc)

  local paraRnnSize = opt.para_rnn_size
  local sentRnnSize = opt.sent_rnn_size
  if opt.rnn_size ~= paraRnnSize + sentRnnSize then
    error('rnn_size should be equal to para_rnn_size + sent_rnn_size')
  end

  -- Compute rnn hidden size depending on hidden states merge action.
  local rnnSize = opt.rnn_size
  if opt.brnn_merge == 'concat' then
    if paraRnnSize % 2 ~= 0 then
      error('in concat mode, para_rnn_size must be divisible by 4')
    end
    if sentRnnSize % 2 ~= 0 then
      error('in concat mode, sent_rnn_size must be divisible by 4')
    end
    paraRnnSize = paraRnnSize / 2
    sentRnnSize = sentRnnSize / 2
  elseif opt.brnn_merge == 'sum' then
    paraRnnSize = paraRnnSize
    sentRnnSize = sentRnnSize
  else
    error('invalid merge action ' .. opt.brnn_merge)
  end

  local sentRnn = onmt.LSTM.new(opt.layers, inputSentSize, sentRnnSize, opt.dropout, opt.residual)
  local paraRnn = onmt.LSTM.new(opt.layers, inputParaSize, paraRnnSize, opt.dropout, opt.residual)

  return onmt.ParaSentEncoder.new(inputSentNetwork, inputParaNetwork, sentRnn, paraRnn, opt.brnn_merge)
end

local function buildParaSentDecoder(opt, dicts, verbose)
  local inputNetwork, inputSize = buildInputNetwork(opt, dicts, opt.pre_word_vecs_dec, opt.fix_word_vecs_dec)
  ...
  return onmt.ParaSentDecoder.new(inputNetwork, rnn, generator, opt.input_feed == 1, opt.sent_rnn_size)
end

local function loadParaSentEncoder(pretrained, clone)
  if clone then
    pretrained = clonePretrained(pretrained)
  end

  return onmt.ParaSentEncoder.load(pretrained)
end

local function loadParaSentDecoder(pretrained, clone)
  if clone then
    pretrained = clonePretrained(pretrained)
  end

  return onmt.ParaSentDecoder.load(pretrained)
end
...
return {
  ...
  buildParaSentEncoder = buildParaSentEncoder,
  buildParaSentDecoder = buildParaSentDecoder,
  loadParaSentEncoder = loadParaSentEncoder,
  loadParaSentDecoder = loadParaSentDecoder
}
